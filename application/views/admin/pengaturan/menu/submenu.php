        ...
        <div class="panel-body">
            ...
            <table class="table table-bordered table-striped mb-none" id="datatable-details">		
                ...
                <tbody>
                    <?php 
                        $no =1;
                        foreach ($getsubmenu as $dt){
                            $status = ($dt->status == 1) ? 'Aktif' : 'Tidak Aktif';
                    ?>
                    <tr >
                        <td><?= $no ?></td>
                        <td><?= $dt->nama_submenu ?></td>
                        ...
                        <td class="actions text-center" >
                            <a href="<?= base_url('menu/submenu/'.$dt->menu_id)?>" class="btn btn-success"><i class="fa fa-list"></i> Sub Menu</a>
                            <a href="<?= base_url('menu/edit-menu/'.$dt->menu_id)?>" class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                            <a href="<?= base_url('menu/delete-menu/'.$dt->menu_id)?>" class="btn btn-danger" onclick="return confirm('Yakin ingin menghapus data ini?')"><i class="fa fa-trash-o"></i></a>
                        </td>
                    </tr>
                    <?php 
                        $no++;
